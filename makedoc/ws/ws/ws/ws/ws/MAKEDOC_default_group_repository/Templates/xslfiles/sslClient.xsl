...
<xsl:text disable-output-escaping="yes"><![CDATA[<!DOCTYPE concept PUBLIC "-//OASIS//DTD DITA Concept//EN" "]]></xsl:text><xsl:value-of select="concat($mdcfg,'/dtd','/concept.dtd')"/><xsl:text disable-output-escaping="yes">"></xsl:text>  
		...
		<concept id="SSLClientMain" xml:lang="en-us">
			<xsl:variable name="processName" select="substring-before($heading,'.sslClientResource')"/>
		<title>
			...
			<navtitle>
				<xsl:choose>
					<xsl:when test="$displayPathInTopicName">
              			<xsl:value-of select="substring-before($relativePath,'.sslClientResource')"/>
            		</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$processName"/>
					</xsl:otherwise>
		...
		<conbody>
			<p>
				<xsl:value-of select="CustomFunctions:initializeTemplate('sslClient.xsl')"/>
			</p>
			<p>
				Section contains description SSL Client "
		...
		<concept id="SSLClientBasic">
			<title>Basic SSL Client configuration</title>
				<conbody>
					<ul>
			...
			<concept id="SSLClientAdvanced">
				<title>Advanced SSL client configuration</title>
					<conbody>
						<ul>
	...
	<xsl:template match="jndi:configuration" mode="basic">
		...
		<li>
			<xsl:choose>
				...
				<xsl:otherwise>
					Keystore Provider as Trust  Store: No keystore provider chosen.
				</xsl:otherwise>	
			</xsl:choose>
		</li>
	...
	<xsl:template match="jndi:configuration" mode="advanced">
		...
		<xsl:if test="@sslCipherStrength='ExplicitCiphers'">
			<li>Explicit Cipher List:
			<xsl:call-template name="getValueFromLiteralModule">
				<xsl:with-param name="valueLiteral" select="@sslExplicitCiphers"/>
				<xsl:with-param name="valueModuleProp" select="substitutionBindings[@template='sslExplicitCiphers']/@propName"/>
				<xsl:with-param name="defaultValue" select="''"/>
			</xsl:call-template>
		</li>
		</xsl:if>
