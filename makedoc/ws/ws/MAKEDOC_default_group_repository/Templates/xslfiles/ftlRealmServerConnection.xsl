...
<!--                                                                              -->
<!-- project         : MakeDoc                                                    -->
<!-- filename        : ftlRealmServerConnection.xsl                               -->
<!-- originator      : lsatin                                                     -->
...
<xsl:text disable-output-escaping="yes"><![CDATA[<!DOCTYPE concept PUBLIC "-//OASIS//DTD DITA Concept//EN" "]]></xsl:text><xsl:value-of select="concat($mdcfg,'/dtd','/concept.dtd')"/><xsl:text disable-output-escaping="yes">"></xsl:text>  
		...
		<concept id="FTLConnectionsMain" xml:lang="en-us">
			<xsl:variable name="processName" select="substring-before($heading,'.ftlResource')"/>
		<title>
			...
			<navtitle>
				<xsl:choose>
					<xsl:when test="$displayPathInTopicName">
              			<xsl:value-of select="substring-before($relativePath,'.ftlResource')"/>
            		</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$processName"/>
					</xsl:otherwise>
					...
					<conbody>
						<p>
							<xsl:value-of select="CustomFunctions:initializeTemplate('ftlRealmServerConnection.xsl')"/>
						</p>
						<p>
							Section contains description of FTL Connections "
		...
		<concept id="FTLConnections">
		<title>FTL Realm Server Connection Configuration</title>
		<conbody>
	...
	<xsl:template match="jndi:configuration">
		<ul>
			<xsl:if test="@realmServerURL != '' or substitutionBindings[@template='realmServerURL']/@propName != ''">
				<li>FTL Realm Server: 
					<xsl:call-template name="getValueFromLiteralModule">
						<xsl:with-param name="valueLiteral" select="@realmServerURL"/>
						<xsl:with-param name="valueModuleProp" select="substitutionBindings[@template='realmServerURL']/@propName"/>
						<xsl:with-param name="defaultValue" select="''"/>
					</xsl:call-template>
				</li>
			</xsl:if>
			<xsl:if test="@secondaryRealmServerURL != '' or substitutionBindings[@template='secondaryRealmServerURL']/@propName != ''">
				<li>Secondary Realm Server: 
					<xsl:call-template name="getValueFromLiteralModule">
						<xsl:with-param name="valueLiteral" select="@secondaryRealmServerURL"/>
						<xsl:with-param name="valueModuleProp" select="substitutionBindings[@template='secondaryRealmServerURL']/@propName"/>
						<xsl:with-param name="defaultValue" select="''"/>
					</xsl:call-template>
				</li>
			</xsl:if>
			<xsl:if test="@applicationName != '' or substitutionBindings[@template='applicationName']/@propName != ''">
				<li>Application Name: 
					<xsl:call-template name="getValueFromLiteralModule">
						<xsl:with-param name="valueLiteral" select="@applicationName"/>
						<xsl:with-param name="valueModuleProp" select="substitutionBindings[@template='applicationName']/@propName"/>
						<xsl:with-param name="defaultValue" select="''"/>
					</xsl:call-template>
				</li>
			</xsl:if>
			<xsl:if test="@applicationInstanceIdentifier != '' or substitutionBindings[@template='applicationInstanceIdentifier']/@propName != ''">
				<li>Application Instance Identifier:
					<xsl:call-template name="getValueFromLiteralModule">
						<xsl:with-param name="valueLiteral" select="@applicationInstanceIdentifier"/>
						<xsl:with-param name="valueModuleProp" select="substitutionBindings[@template='applicationInstanceIdentifier']/@propName"/>
						<xsl:with-param name="defaultValue" select="''"/>
					</xsl:call-template>
				</li>
			</xsl:if>
			<xsl:if test="@username != '' or substitutionBindings[@template='username']/@propName != ''">
				<li>Username:
					<xsl:call-template name="getValueFromLiteralModule">
			...
			<xsl:if test="@password != '' or substitutionBindings[@template='password']/@propName != ''">
				<li>Password:
				<xsl:choose>
				<xsl:when test="@password !='' or substitutionBindings[@template='password']/@propName !=''"> **** </xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="getValueFromLiteralModule">
