...
<!--                                                                              -->
<!-- project         : MakeDoc                                                    -->
<!-- filename        : javaGlobalInstance.xsl                                     -->
<!-- originator      : lsatin                                                     -->
...
<xsl:text disable-output-escaping="yes"><![CDATA[<!DOCTYPE concept PUBLIC "-//OASIS//DTD DITA Concept//EN" "]]></xsl:text><xsl:value-of select="concat($mdcfg,'/dtd','/concept.dtd')"/><xsl:text disable-output-escaping="yes">"></xsl:text>  
	...
	<concept id="JavaGlobalInstanceMain" xml:lang="en-us">
			<xsl:variable name="processName" select="substring-before($heading,'.javaGlobalInstanceResource')"/>
		<title>
			...
			<navtitle>
				<xsl:choose>
					<xsl:when test="$displayPathInTopicName">
              			<xsl:value-of select="substring-before($relativePath,'.javaGlobalInstanceResource')"/>
            		</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$processName"/>
					</xsl:otherwise>
					...
					<conbody>
						<p>
							<xsl:value-of select="CustomFunctions:initializeTemplate('javaGlobalInstance.xsl')"/>
						</p>
						<p>
							Section contains description of Java Global Instance "
		...
		<concept id="JavaGlobalInstance">
			<title>Java Global Instance</title>
				<conbody>
					<ul>
	...
	<xsl:template match="jndi:configuration">
		...
		<xsl:for-each select="javaConstructorParameter">
			<li>
				<xsl:value-of select="@paramName"/> => <xsl:value-of select="@paramValue"/> (<xsl:value-of select="@paramType"/>)
			</li>
		</xsl:for-each>
		<xsl:if test="@invokeCleanupMethod != ''">
			<li>
				Invoke Cleanup Method:
				<xsl:value-of select="@invokeCleanupMethod"/>
			</li>		
			<li>
				Cleanup Method:
				<xsl:value-of select="@cleanupMethodName"/>
			</li>	
		</xsl:if>
	</xsl:template>
