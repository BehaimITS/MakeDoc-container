...
<xsl:text disable-output-escaping="yes"><![CDATA[<!DOCTYPE concept PUBLIC "-//OASIS//DTD DITA Concept//EN" "]]></xsl:text><xsl:value-of select="concat($mdcfg,'/dtd','/concept.dtd')"/><xsl:text disable-output-escaping="yes">"></xsl:text>  
		...
		<concept id="SFTPConnectionsMain" xml:lang="en-us">
			<xsl:variable name="processName" select="substring-before($heading,'.sftpResource')"/>
		<title>
			...
			<navtitle>
				<xsl:choose>
					<xsl:when test="$displayPathInTopicName">
              			<xsl:value-of select="substring-before($relativePath,'.sftpResource')"/>
            		</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$processName"/>
					</xsl:otherwise>
					...
					<conbody>
						<p>
							<xsl:value-of select="CustomFunctions:initializeTemplate('sftpConnection.xsl')"/>
						</p>
						<p>
							Section contains description of SFTP Connections "
		...
		<concept id="SFTPConnections">
		<title>SFTP Connection Configuration</title>
		<conbody>
			<ul>
	...
	<xsl:template match="jndi:configuration" mode="ConnectionConfiguration">
		...
		<li>
			...
			<xsl:call-template name="getValueFromLiteralModule">
				...
				<xsl:with-param name="defaultValue" select="'22'"/>
			</xsl:call-template>
		</li>
		<li>
			User Name: 
		...
		<li>
			Private Key: 
			<xsl:call-template name="getValueFromLiteralModule">
				<xsl:with-param name="valueLiteral" select="@privKey"/>
				<xsl:with-param name="defaultValue" select="''"/>
			</xsl:call-template>
		</li>
		<li>
			...
			<xsl:call-template name="getValueFromLiteralModule">
				<xsl:with-param name="valueLiteral" select="@privKeypassword"/>
				<xsl:with-param name="defaultValue" select="''"/>
			</xsl:call-template>
		</li>
		<xsl:if test="@strictHostKeyCheck = 'true'">
			<li>
				Strict Hostkey Check: 
				<xsl:call-template name="getValueFromLiteralModule">
					<xsl:with-param name="valueLiteral" select="@strictHostKeyCheck"/>
					<xsl:with-param name="defaultValue" select="''"/>
				</xsl:call-template>
			</li>
			<li>
				Known Host File: 
				<xsl:call-template name="getValueFromLiteralModule">
					<xsl:with-param name="valueLiteral" select="@knownHostFile"/>
					<xsl:with-param name="defaultValue" select="''"/>
				</xsl:call-template>
			</li>
		</xsl:if>
