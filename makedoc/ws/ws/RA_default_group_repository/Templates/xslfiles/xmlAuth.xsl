...
<xsl:stylesheet version="1.0" 
	...
	exclude-result-prefixes="xpdl2 ncname xpdExt java CustomFunctions CustomFunctionsEagle PrimitiveTypeFacets jndi str">
...
<xsl:text disable-output-escaping="yes"><![CDATA[<!DOCTYPE concept PUBLIC "-//OASIS//DTD DITA Concept//EN" "]]></xsl:text><xsl:value-of select="concat($mdcfg,'/dtd','/concept.dtd')"/><xsl:text disable-output-escaping="yes">"></xsl:text>  
		...
		<concept id="XMLAuthenticationMain" xml:lang="en-us">
			<xsl:variable name="processName" select="substring-before($heading,'.authxml')"/>
		<title>
			...
			<navtitle>
				<xsl:choose>
					<xsl:when test="$displayPathInTopicName">
              			<xsl:value-of select="substring-before($relativePath,'.authxml')"/>
            		</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$processName"/>
					</xsl:otherwise>
					...
					<conbody>
						<p>
							<xsl:value-of select="CustomFunctions:initializeTemplate('xmlAuth.xsl')"/>
						</p>
						<p>
							Section contains description of XML authentication " 
		...
		<concept id="XMLAuth">
		<title>XML Authentication</title>
		<conbody>
			<ul>	
						<li>XML File:
							<xsl:call-template name="getValueFromLiteralModule">
								<xsl:with-param name="valueLiteral" select="jndi:configuration/@fileLocation"/>
								<xsl:with-param name="valueModuleProp" select="jndi:configuration/substitutionBindings[@template='fileLocation']/@propName"/>
